...
#
# Reads the hook event JSON from stdin. If the tool was a `git push` whose
# target ref has an open PR, emit additionalContext reminding the agent to
# refresh the PR title/description if the pushed scope changed.
#
...
# Only fire for real `git push` invocations (not `git status` etc. mentioning push).
case "$command_str" in
  *"git push"*|*"git  push"*) ;;
  *) exit 0 ;;
esac
...
reminder="Just pushed branch \`${branch}\` which has open PR #${pr_number}: \"${pr_title}\"
${pr_url}
...
If you want to watch CI for this PR, the \`ci-watch\` skill is available."
