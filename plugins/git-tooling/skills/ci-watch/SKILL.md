...
Watch GitHub PR status through to merge without manually polling. This skill invokes the `Monitor` tool with `ci-watch.py`, which emits one notification per state transition and exits only when every watched PR is merged, closed, or gone. CI completion and review status are reported as intermediate milestones — a `READY` flag appears when a PR is mergeable (all checks green, reviews clear, no conflicts) **and** GitHub affirmatively reports it mergeable: `mergeable == MERGEABLE` (not merely "not conflicting") and `mergeStateStatus` is neither blocked nor still being computed. This catches required reviews, ruleset rules, and conversation-resolution gates even when no tracked signal explains them, and avoids a misleading early `READY` in the window right after a push before GitHub has computed mergeability.

The watcher also reports **draft transitions** (`DRAFTED`/`UNDRAFTED`), flags a PR with more than 100 checks (`TRUNC`, counts may be partial), and surfaces a one-time `WARN` if a PR stays unreadable across several polls instead of silently idling. Many PRs (across many repos) are fetched in batched GraphQL requests, and polling backs off automatically when the API errors.
...
| `PR #48: DRAFTED,P=5,F=0,W=0,DRAFT(by github-actions)` | The PR just flipped **to** draft this poll — `DRAFTED` is a transition marker prepended to the line (the `DRAFT(by …)` token still shows current state) |
| `PR #48: UNDRAFTED,P=5,F=0,W=0,READY` | The PR just flipped **back to ready-for-review** — `UNDRAFTED` is the transition marker; auto-reviews resume |
| `PR #48: P=5,F=0,W=0,TRUNC` | The head commit has >100 checks; the `P/F/W` counts cover only the first page and may be incomplete. `READY` is suppressed while truncated. Inspect with `gh pr checks <pr>` |
| `PR #48: WARN unreadable after 3 consecutive API errors; still retrying` | The PR couldn't be read for several polls (rate limit / outage). The watcher keeps retrying with exponential backoff — emitted once per error streak, not every poll |
...
- **`DRAFTED` / `UNDRAFTED`** — the PR's draft state just flipped this poll. `DRAFTED` = converted to draft (auto-reviews pause; usually `draft_on_blocking` — see the `DRAFT(by …)` token on the same line). `UNDRAFTED` = returned to ready-for-review (reviews resume). These are transition markers, distinct from the steady-state `DRAFT(by …)` token.
- **`TRUNC`** — the head commit has >100 checks, so the `P/F/W` counts are partial and `READY` is withheld. Use `gh pr checks <pr> -R <owner/repo>` for the authoritative list.
- **`WARN unreadable …`** — the PR has been unreadable for several polls (rate limit, network, or a token-scope problem). The watcher keeps retrying with backoff; if it persists, check `gh auth status` and rate limits (`gh api rate_limit`).
...
- The script is null-safe for PRs with no checks at all (reports `P=0,F=0,W=0` and only adds `READY` once GitHub confirms `mergeable == MERGEABLE`).
- **Batched fetches:** PRs are queried in aliased GraphQL requests (up to 10 per request), so watching many PRs across many repos costs a handful of API calls per poll rather than one per PR. Partial failures are handled per-PR — one unreadable PR doesn't blank the rest.
- When the token can't read some check-runs (app-authored checks like Copilot's review surface as `null` rollup nodes — common with fine-grained PATs), the script recovers the `P`/`F`/`W` counts **and the `FAIL[…]` names** from the Actions API (workflow run conclusions) for that commit instead of going silent on the PR. The Actions-API fallback paginates, so a commit with many reruns isn't truncated. Other PR-level signals (`U`, `CR`, `RR`, `BLOCKED`, merge state) are unaffected.
...
- Checks gated on manual approval (`action_required`) and start-up failures are counted as pending/failing rather than ignored, so they can't let a premature `READY` through.
- **Error backoff:** on repeated API errors, the poll interval backs off exponentially (capped at 5 min) and emits a one-time `WARN` per PR after 3 consecutive failures, so a rate-limit or outage is neither hammered nor silently swallowed. A fully-READY PR keeps polling at the normal interval — it's still waiting on a human to merge, not something to ease off watching.
