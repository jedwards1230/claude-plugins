...
| `PR #48: P=5,F=0,W=0,U=3,DRAFT(by github-actions)` | The PR was converted **back to draft** — usually our `claude-pr-review` reusable's `draft_on_blocking`, which drafts a PR when a review posts blocking comments so re-reviews pause until the author resolves them and marks it ready again. `by <login>` names whoever drafted it (`github-actions` ⇒ the review bot; a human login ⇒ a manual draft). `DRAFT` alone = drafted but the actor wasn't in the timeline. READY never appears while drafted. |
