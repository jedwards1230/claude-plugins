...
- **Adaptive cadence:** once all watched PRs reach READY, the poll interval doubles (e.g. 30s → 60s) to reduce API usage while waiting for merge. On repeated API errors it instead backs off exponentially (capped at 5 min) and emits a one-time `WARN` per PR after 3 consecutive failures, so a rate-limit or outage is neither hammered nor silently swallowed.
