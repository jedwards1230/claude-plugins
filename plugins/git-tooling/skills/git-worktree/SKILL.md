...
**Hook enforcement**: This plugin ships a `PreToolUse(Bash)` hook that denies `git commit` on a repo's default branch. If you see a deny message referencing `GIT_TOOLING_ALLOW_DEFAULT_BRANCH_COMMIT`, jump to Workflow 2 (Create a New Branch and Worktree) before retrying the commit.
