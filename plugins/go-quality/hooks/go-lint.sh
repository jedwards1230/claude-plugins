...
if $GOLANGCI run --timeout 60s $PACKAGES >/tmp/go-quality-lint-out.txt 2>&1; then
  exit 0
fi

# Handle v1/v2 config mismatch gracefully
if grep -q "configuration file for golangci-lint v2 with golangci-lint v1" /tmp/go-quality-lint-out.txt 2>/dev/null; then
...
cat /tmp/go-quality-lint-out.txt >&2
exit 2
