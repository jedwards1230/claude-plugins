...
- **Integration-branch flow** (opt-in, on user request): the multi-PR topology for
  work too big for one PR — agents own their CI loop and merge into an integration
  branch; the human reviews and merges exactly one tracking PR at the end.
...
|-------|------------|----------------|
| `orchestrate` | About to fan out / nest subagents | Readiness gate, delegate-or-not, depth/breadth, brief files, context-packing, fork-vs-fresh, opt-in integration-branch flow, return contracts, recovery, patterns, gotchas |
