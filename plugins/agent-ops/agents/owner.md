...
- **Never merge into the default branch.** Promoting an integration/staging branch to the default branch is the human's review gate, always — no brief delegates it. Merging a *feature* PR into an integration branch is different: that grant is delegable, so take it only when your brief names **you** as the holder. Absent that, open the PR and report it. Your base branch comes from your brief, not your own call. Report each PR URL the moment it opens — don't make anyone ask "are they open? where are the links?".
